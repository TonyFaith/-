#-*- coding:utf-8 -*-
# 训练
# 需要很长的时间训练，这还是很小的数据集，如果有百GB的数据，没十天半个月也训练不完
...
def read_data(source_path, target_path, max_size=None):
    ...
    with tf.gfile.GFile(source_path, mode='r') as source_file:
        with tf.gfile.GFile(target_path, mode='r') as target_file:
            source, target = source_file.readline(), target_file.readline()  # 每次读取一行数据
            counter = 0
            # print(source)
            # print(target)
            while source and target and (not max_size or counter < max_size):
                counter += 1
                source_ids = [int(x) for x in source.split()]  # split()当不带参数时以空格进行分割
                target_ids = [int(x) for x in target.split()]
                target_ids.append(EOS_ID)
                for buckets_id, (source_size, target_size) in enumerate(buckets):
                    # print(buckets_id,(source_size,target_size))
                    # print(len(source_ids))
                    # print(len(target_ids))
                    # print("####")
                    if len(source_ids) < source_size and len(target_ids) < target_size:
                        data_set[buckets_id].append([source_ids, target_ids])
                        break
                source, target = source_file.readline(), target_file.readline()  # 接着读取下一行数据
    return data_set


model = seq2seq_model.Seq2SeqModel(source_vocab_size=vocabulary_encode_size, target_vocab_size=vocabulary_decode_size,
                                   buckets=buckets, size=layer_size, num_layers=num_layer, max_gradient_norm=5.0,
...
with tf.Session(config=config) as sess:
    ...
    while True:
        ...
        _, step_loss, __ = model.step(sess, encoder_inputs, decoder_inputs, target_weights, bucket_id, False)
